...
```{r bills, fig.cap="Bills of penguins", echo = FALSE,fig.width=8, fig.height=4}
age_group_diverging <- data %>%
...
age_group_distribution <- age_group_diverging %>% distinct(population_count, .keep_all = TRUE)
age_group_distribution %>% 
  ggplot(aes(fill = gender, y = population_count, x = demographic_info)) +
  ...
  labs(
    title = "Survey Respondent Distribution",
    x = "age group",
...
data1 <- data %>% filter( demographic_type == "age group")

scatterPlot1 <- ggplot(data1, aes(demographic_info, chances_of_getting_aids, color = survey_answer)) +
  geom_point() + geom_jitter()+ facet_wrap(~gender)+
